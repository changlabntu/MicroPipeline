...
def get_fusion(img_path, deconv, osize, destination):
    ...
    tiff.imsave('xfftrori.tif', torch.real(xfft)[:,:,:,0].numpy())
...
if __name__ == '__main__':
    if 1:
        ...
        #img_path = root + 'descar_k7_100.tif'
        img_path = root + 'original_cropped.tif'
    elif 0:
        root = '/media/ghc/GHc_data2/BRC/4xfly/'
        img_path = root + 'roiA.tif'
    elif 0:
        root = '/media/ghc/GHc_data2/BRC/64246_VMAT_complete_560/Fly0B/'
        img_path = root + 'xyori.tif'

    deconv = root + 'deconv/'

    tif = tiff.imread(img_path)[0:1, :, :]#[500:510, :1024, 1024:]
    osize = tif.shape[1]
    ...
    rates = [x / 100 for x in list(range(10, 101, 2))]
    #rates = [x / 100 for x in list(range(100, 10, -2))]
